				 ...
				 NewsBean newsBean = new NewsBean();
		            ...
		            if(method.equals("ADDNEWS")){//增加新闻
						...
						request.getRequestDispatcher(sysdir+"/news/edit.jsp").forward(request, response);
							...
							int flag = newsBean.addNews(title,fenlei,content, username2);
		            ...
		            else if(method.equals("editnews")){//修改
		            	String id = Filter.escapeHTMLTags(request.getParameter("id").trim());
		            	String title = Filter.escapeHTMLTags(request.getParameter("title").trim());
		            	String fenlei = request.getParameter("fenlei");
						String content = request.getParameter("infoContent");
							int flag = newsBean.updateNews(Integer.parseInt(id), title,fenlei, content, username2);
							if(flag == Constant.SUCCESS){
		            ...
		            else if(method.equals("DELNEWS")){//删除 来自普通页面index.jsp
		            ...
		            else if(method.equals("addFenlei")){
		            	...
		            	if(member==null){
		            		request.setAttribute("message", "请登录后再评论！");
